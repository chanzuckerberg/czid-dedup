...
fn single<T: fastx::Record, R: Iterator<Item = Result<T, std::io::Error>>, S: fastx::Writer<T>, U: std::io::Write>(records: R, mut writer: S, mut clusters: clusters::Clusters<U>) -> Result<clusters::Clusters<U>, Box<dyn Error>> {
    for result in records {
        let record = box_bail!(result);
        box_bail!(record.check().map_err(|err| simple_error::simple_error!(err)));
...
fn pair<T: fastx::Record, R: Iterator<Item = Result<T, std::io::Error>>, S: fastx::Writer<T>, U: std::io::Write>(records: paired::PairedRecords<T, R>, mut writer_r1: S, mut writer_r2: S, mut clusters: clusters::Clusters<U>) -> Result<clusters::Clusters<U>, Box<dyn Error>> {
    for result in records {
        let (record_r1, record_r2) = unwrap_or_return!(result);
    
        box_bail!(record_r1.check().map_err(|err| simple_error::simple_error!(err)));
        box_bail!(record_r2.check().map_err(|err| simple_error::simple_error!(err)));
...
fn run_dedup<T: Into<std::ffi::OsString> + Clone, R: IntoIterator<Item = T>>(args: R) -> Result<clusters::Clusters<File>, Box<dyn Error>> {
    let matches = App::new(clap::crate_name!())
                        ...
                        .arg(Arg::with_name("inputs")
                            ...
                            .takes_value(true)
                            .required(true))
                        .arg(Arg::with_name("deduped-outputs")
                            .short("o")
                            ...
                            .takes_value(true)
                            .required(true))
                        .arg(Arg::with_name("cluster-output")
                            .short("c")
                            .long("cluster-output")
                            .help("Output cluster file")
                            .takes_value(true)
                            .default_value("clusters.csv"))
                        .arg(Arg::with_name("prefix-length")
                            .short("l")
                            .long("prefix-length")
    ...
    let prefix_length_opt = matches.value_of("prefix-length").map( |n| n.parse::<usize>().unwrap() );
    ...
    let clusters = clusters::Clusters::from_file(cluster_output, prefix_length_opt, bytes / 400).unwrap();

    match fastx::fastx_type(input_r1).unwrap() {
        fastx::FastxType::Fasta => dedup!(fasta, fastx::FastxType::Fasta, input_r1, output_r1, inputs, outputs, clusters),
        fastx::FastxType::Fastq => dedup!(fastq, fastx::FastxType::Fastq, input_r1, output_r1, inputs, outputs, clusters),
        fastx::FastxType::Invalid => Err(Box::new(simple_error::simple_error!("input file is not a valid FASTA or FASTQ file")) as Box<dyn Error>),
    }
}
...
mod test {
    ...
    fn test_run_dedup_single() {
        ...
        let args = ["executable", "-i", &input_path, "-o", &output_path, "-c", &cluster_path];
